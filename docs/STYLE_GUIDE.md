...
- **Route path segments use the app's existing camelCase resource naming.** Do not default to kebab-case for new frontend routes. Match the existing resource style (`payPeriod`, `payrollReport`, `allocationReport`, etc.); for example, the Timesheet Folders management route is `/client/:clientId/timesheetFolders`, not `/client/:clientId/timesheet-folders`.
...
- **Nested layout routes for a group of sibling pages under one resource** (decided 2026-07-11, see `docs/PAY_PERIOD_PAGES.md`): when several pages are really different views of the same one thing (e.g. a pay period's Timesheet Status / Payroll Report / Allocation Report), use a parent layout route rendering shared persistent chrome (a summary card + a `Tabs` switcher, not `Breadcrumbs` — these are siblings, not a hierarchical trail) + `<Outlet/>`, same relationship `AppLayout` has to `Header`/`Outlet`, one level deeper. **This does not relax independent fetches (§3)**: the parent fetches only what it needs for its own chrome; each child page still fetches its own data independently. The one narrow exception is a callback (not fetched data) passed down via `<Outlet context={...}>` when a child's action needs to invalidate something only the parent fetches (e.g. a child's Generate action flips a status field the parent's fetch owns) — passing a function down to let a child ask the parent to refetch its own thing isn't the same as sharing fetched data between siblings.
...
- **`resolveErrorMessage`** (2026-07-11) — the generated API client throws a `ResponseError` carrying only the raw HTTP `Response` on a non-2xx result; the backend's own `{ error, message }` body has to be parsed out separately (`await error.response.json()`). This util does that, falling back to a caller-supplied message for anything else (network failure, etc.). Exists because the backend returns genuinely useful validation messages (e.g. "No Complete timesheets found...") that are worth surfacing verbatim rather than replacing with a generic string.

## 6b. Generic hooks

- **`src/hooks/<hookName>.ts`, one hook per file** — same convention as `utils/` (§6a), for reusable hooks that aren't tied to a specific state domain. Domain-specific hooks (e.g. `useSelectedClient`, which reads `client` context) still live under `src/state/<domain>/`, per §3 — `src/hooks/` is only for cross-cutting hooks with no domain ownership.
- First example: `useFetchByKey` (2026-07-11) — extracted after the exact same "fetch keyed by a URL param, guard against stale/out-of-order responses" logic was hand-written in both `ClientSummary.tsx` and `PayPeriodDashboard.tsx`. Guards two races that come from a component being reused across keys without unmounting (e.g. navigating between two clients or two pay periods): an in-flight response for a stale key landing after a newer one already resolved (guarded via a `cancelled` flag), and briefly rendering the previous key's data under the new key before its fetch resolves (guarded by comparing the key the current data belongs to against the current key). Each page still owns its own loading/error/redirect _display_ logic — the hook only owns the fetch mechanics. Also exposes `refetch()` (added 2026-07-11) — forces the same key to be re-fetched, for when a mutation elsewhere (e.g. generating a payroll report, which flips the pay period's own `status` server-side) invalidates data this hook already loaded, without waiting for `key` itself to change.
- **`useAsyncAction`** (2026-07-11) — wraps a button-triggered async action (Generate Timesheets, Generate Payroll Report, Close Pay Period, etc.) with the disable-while-in-flight + loading indicator baseline every such control needs (§8), plus `resolveErrorMessage` for surfacing the backend's real validation message on failure. Extracted before a second call site even existed — four known upcoming buttons (`8.6`, `8.8`, `8.10`, `10.3` in `TICKETS.md`) all need the identical mechanics, so this is the same "wait for real, known repetition" bar `useFetchByKey` cleared, just applied slightly ahead of the second literal usage rather than after it.
